...
getMin start [] word = start ++ (getPrefixes start)
getMin start rest word = do
			...
			else if isBiggest posWords word && (dupWeight ord word) == False then start
			...
			else if isSecondHighest ord word && (dupWeight ord word) == False then start ++ "^"
			else getMin (start ++ [head rest]) (tail rest) word


getPrefixes :: [Char] -> [Char]
getPrefixes x = do
			let posWords = getKeyPressedWords (getWordDigits x)
			let sort = (orderWeight posWords)
			let s = map fst sort
			let le = elemIndex x s
			concat (replicate ((fromJust le)) "^") 
...
{- Checks if the words weight exists more then once, returns true if there is 
   more then one of the weight else false -}
